...
namespace CustomEffect.SampleProviders
{
    /// <summary>
    /// Signal Generator
    ...
    public sealed class SignalGenerator
    {
        ...
        private readonly AudioEncodingProperties audioEncodingProperties;
        ...
        public SignalGenerator(AudioEncodingProperties audioEncodingProps)
        {
            phi = 0;
            audioEncodingProperties = audioEncodingProps;
   
            // Default
            Type = SignalGeneratorType.Sin;
            Frequency = 440.0;
            Gain = 1;
            PhaseReverse = new bool[audioEncodingProperties.ChannelCount];
            SweepLengthSecs = 2;
        }
        ...
        public float[] Read(int offset, int count)
        {
            ...
            // Complete Buffer
            for (int sampleCount = 0; sampleCount < (count / audioEncodingProperties.ChannelCount); sampleCount++)
            {
                switch (Type)
                {
                    case SignalGeneratorType.Sin:
                        ...
                        multiple = TwoPi * Frequency / audioEncodingProperties.SampleRate;
                    ...
                    case SignalGeneratorType.Square:
                        ...
                        multiple = 2 * Frequency / audioEncodingProperties.SampleRate;
                    ...
                    case SignalGeneratorType.Sweep:
                        ...
                        // Sweep Generator
                        double f = Math.Exp(FrequencyLog + (nSample * (FrequencyEndLog - FrequencyLog)) / (SweepLengthSecs * audioEncodingProperties.SampleRate));

                        multiple = TwoPi * f / audioEncodingProperties.SampleRate;
                        phi += multiple;
                        sampleValue = Gain * (Math.Sin(phi));
                        nSample++;
                        if (nSample > SweepLengthSecs * audioEncodingProperties.SampleRate)
                        {
                            nSample = 0;
                            phi = 0;
                        }
                ...
                for (int i = 0; i < audioEncodingProperties.ChannelCount; i++)
                {
                    if (PhaseReverse[i])
